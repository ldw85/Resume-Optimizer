export default function Trust() {
    try {
        return (
            <section data-name="trust" className="py-20 bg-gray-50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center">
                        <h2 className="text-3xl font-extrabold text-gray-900 sm:text-4xl">
                            Sicher und Zuverlässig
                        </h2>
                        <p className="mt-4 text-xl text-gray-600">
                            Wir schätzen Ihre Privatsphäre und Sicherheit
                        </p>
                    </div>

                    <div className="mt-12 grid grid-cols-1 gap-8 lg:grid-cols-3">
                        <div className="trust-card p-6 rounded-xl">
                            <div className="w-12 h-12 bg-indigo-100 rounded-lg flex items-center justify-center mb-4">
                                <i className="fas fa-shield-alt text-indigo-600 text-xl"></i>
                            </div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                Datensicherheit
                            </h3>
                            <p className="text-gray-600">
                                Alle Daten werden in Echtzeit verarbeitet und nicht gespeichert
                            </p>
                        </div>
                        
                        <div className="trust-card p-6 rounded-xl">
                            <div className="w-12 h-12 bg-indigo-100 rounded-lg flex items-center justify-center mb-4">
                                <i className="fas fa-lock text-indigo-600 text-xl"></i>
                            </div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                Datenschutz
                            </h3>
                            <p className="text-gray-600">
                                Strenge Datenschutzmechanismen zur Gewährleistung der Sicherheit persönlicher Daten
                            </p>
                        </div>
                        
                        <div className="trust-card p-6 rounded-xl">
                            <div className="w-12 h-12 bg-indigo-100 rounded-lg flex items-center justify-center mb-4">
                                <i className="fas fa-check-circle text-indigo-600 text-xl"></i>
                            </div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                Zuverlässiger Service
                            </h3>
                            <p className="text-gray-600">
                                Professioneller technischer Team-Support zur Gewährleistung der Servicestabilität
                            </p>
                        </div>
                    </div>
                </div>
            </section>
        );
    } catch (error) {
        console.error('Trust component error:', error);
        // reportError(error); // Assuming reportError is not globally available or needed here
        return null;
    }
}
